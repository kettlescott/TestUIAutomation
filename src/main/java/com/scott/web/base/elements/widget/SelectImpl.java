...
public class SelectImpl extends ElementImpl implements Select {
   ...
   * @throws NoSuchElementException If no matching option elements are found or the elements are not
   *     visible or disabled.
   */
  ...
  /**
   * Select the option atPage the given index. This is done by examing the "index" attribute of an
  ...
  private String escapeQuotes(String toEscape) {
    if (toEscape.indexOf("\"") > -1 && toEscape.indexOf("'") > -1) {
      ...
      for (int i = 0; i < substrings.length; i++) {
        ...
        quoted.append(
            ((i == substrings.length - 1) ? (quoteIsLast ? ", '\"')" : ")") : ", '\"', "));
      }
  ...
  private enum State {
    ...
    private void checkState(String searchCriteria) {
      switch (this) {
        ...
        case NOT_FOUND:
          throw new NoSuchElementException("Cannot locate option with " + searchCriteria);
        case SELECTED:
          // DO_NOTHING;
      }
    }
